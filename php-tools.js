...
async function createController(context, e, controllerName = null, controllerDescription = '') {
    if (!e) {
    ...
    if (typeof e === 'string') {
        path = e;
    } else if (e?.data?.obj?.controllersPath !== undefined) {
    ...
    if (folderPath.indexOf('/vendor/') !== -1) {
        ...
        if (!controllerDescription) {
            controllerDescription = await vscode.window.showInputBox({
                title: vscode.l10n.t('Controller description:'),
                placeHolder: vscode.l10n.t('Please, describe your controller'),
                ignoreFocusOut: true
            });
        }
...
async function createControllerAction(context, e, controllerActionName, controllerActionDescription) {
    ...
    if (typeof e === 'string') {
        ...
        folderPath = editor?.document?.uri?.fsPath;
    } else {
        folderPath = e.fsPath;
    }
